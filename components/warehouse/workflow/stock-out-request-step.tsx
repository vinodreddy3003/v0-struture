...
export function StockOutRequestStep({ onAddRequest }: StockOutRequestStepProps) {
  const { requests, approveRequest, rejectRequest, startWorkflow } = useStockOutStore();
  ...
  const handleAddRequest = () => {
    const newRequest: Omit<StockOutRequest, "id" | "status" | "pickingDetails"> = {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between border-b border-border pb-3">
        <h3 className="text-sm font-semibold text-foreground">Stock Out Requests</h3>
        <button
          onClick={() => setShowForm(!showForm)}
          className="inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-white bg-amber-600 hover:bg-amber-700 transition-colors"
        >
          <Plus size={14} />
          New Request
        </button>
      ...
      {otherRequests.length > 0 && (
        <div className="border-t border-border pt-3">
          <details className="text-xs">
            <summary className="font-medium text-foreground cursor-pointer hover:text-amber-600">
